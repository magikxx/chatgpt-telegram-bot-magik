...
# napraw ten błąd Traceback (most recent call last): NBameError: name 'User' is not defined
    
    def get_response(self, user, user_message, conversation_context=None):
        if user not in self.userDict:
            self.userDict[user] = User()
        self.userDict[user].addMessage(user_message)
        answer = self.__sendMessage(user, self.userDict[user].messageList, self.userDict[user].context, conversation_context)
        self.userDict[user].addMessage(answer)
        return answer
    ...
    def __sendMessage(self, user, messageList, conversation_context=None):  # dodano argument conversation_context
        if conversation_context is None:
            conversation_context = self.userDict[user].context
        answer = self.openai_parser.get_response(user, messageList, conversation_context=conversation_context)  # dodano argument conversation_context
        self.userDict[user].context = conversation_context  # zaktualizowano kontekst rozmowy użytkownika
        return answer
