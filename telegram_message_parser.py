...
stripe.api_key = "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"
...
class TelegramMessageParser:
    ...
    async def chat_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not self.check_user_allowed(str(update.effective_user.id)):
            await context.bot.send_message(
                ...
                text="Извините, у вас нет разрешения на использование этого бота. Свяжитесь с владельцем бота, чтобы получить дополнительную информацию."
            )
            return
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        button = InlineKeyboardButton("Kliknij mnie", callback_data="buy")
        keyboard = InlineKeyboardMarkup([[button]])
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="<b>Witaj!</b> Jesteś Twoim asystentem [https://dsadas.pl](fdfds)",
            parse_mode="Markdown",
